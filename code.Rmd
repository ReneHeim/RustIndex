...
install.packages(c("cowplot", "gdata", "glmulti", "hsdar", "plyr",
                   ...
                   "tidyverse", "VSURF", "reshape2", "caret"))
```
...
contains a set of important features with no redundancy. Pls refer to __(CITE)__ 
for more information on the differents sets. For our analysis, we want to find 
the  unique wavebands among all ten selections. Therefore a result list was 
created (2), we looped through each single object (3) to assign them to the 
result list (band.vectors). This list was turned into a single object (4) and 
all unique wavebands were selected (5).
```{r extractbands}
...
_glmulti_ result to find coefficients for the model. We save (4) and reload (5)
...
Eq. 2 and 3 (Heim et al., 2018b). Additionally, to summarize the model 
...
We use the _hsdar_ pkg to define spectral indices. The PRI and MCARI already 
exist in the package and we add the NBNDVI (1) and the LMMR (2). We store them 
all in a vector to process them that way (3).
```{r indices}
...
After the indices have been defined we can use them to convert our spectral data
...
a logistic regression model (train) and validate this model on an isolated 
sample (test). We also define classification parameters (4). For classification 
...
First we build a df using the fitted values of the training process for each 
index (1-5) and add useful names for each column (6).
```{r datatrain}
...
Finally, we can visualize the spectral signature for each classification group
(<Treated> and <Untreated>) and show the most important wavebands. First, we 
reformat our wide df into a long df (1). In a second step, we get the selected 
...
Now we can plot the spectral signatures:
...
We combine the last two plots, as shown in the article and save the plot:
